class GitHub
  ...
  def get_commit_history(repository, commit_number)
    url = URI("https://api.github.com/repos/#{@org}/#{repository}/pulls/#{commit_number}/commits")
  ...
  def post_comment(link, comment)
    ...
    req.basic_auth("token", @auth_token)
    req["content-type"] = "application/json"
    finalComment = ":moyai: Gictator[almost reliable] says: #{comment}\n\n"
    finalComment += "If you think there is a bug in Gictator's judgement please "
